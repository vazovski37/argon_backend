...
-- NOTE: The seed data below requires tables created by Flask-Migrate.
-- These INSERTs are wrapped in a DO block that checks if tables exist first.
-- Run 'flask db upgrade' inside the backend container after first startup
-- to create the tables, then restart to seed data.

DO $$
BEGIN
    -- Only seed if the tables already exist (created by Flask-Migrate)
    ...
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'quests') THEN
